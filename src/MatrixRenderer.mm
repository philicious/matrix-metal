...
static const int kHorizontalOverscanCols = 8;
static const float kHorizontalDensityScale = 1.35f;
...
- (void)setupStateForSize:(int)w height:(int)h {
    ...
    _textX = (int)std::ceil(kTextY * ((float)w / (float)h) * kHorizontalDensityScale);
    if (_textX & 1) _textX++;
    if (_textX < 90) _textX = 90;
    _textX += kHorizontalOverscanCols;
    if (_textX & 1) _textX++;
...
- (void)drawInMTKView:(MTKView *)view {
    ...
    MTLViewport vp = {0.0, 0.0, view.drawableSize.width, view.drawableSize.height, 0.0, 1.0};
    [enc setViewport:vp];
